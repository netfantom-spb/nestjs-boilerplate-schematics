...
import { ModuleOptions } from './rabbitmq-producer.schema';
...
function transform(source: ModuleOptions): ModuleOptions {
  ...
  target.name = normalizeToKebabOrSnakeCase(`${location.name}-producer`);
  target.path = normalizeToKebabOrSnakeCase(`./modules/rabbitmq/producers/${location.path}`);
